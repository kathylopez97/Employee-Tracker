...
cfonts.say("Employee Tracker for Space Force", {
...
// Functions of array of  questions for employee tracker
function start() {
  inquirer
    .prompt({
      type: "list",
...
// an array of questions to select new role from an department
  ...
  connection.query("SELECT id, title FROM roles", (error, results) => {
    ...
    connection.query(
      ...
      (error, results) => {
        ...
        // an array of inquirer prompt questions to select employee manager
        inquirer
          .prompt([
            {
              type: "input",
    ...
    inquirer
      ...
      .then((answer) => {
        ...
        connection.query(query, [answer.departmentId], (err, res) => {
          ...
          // restart the employee tracker application
          start();
        });
      });
  });
...
// This close the connection when the application exits
process.on("exit", () => {
  connection.end();
});
